...
print("\n自动去除共线性后的因子：", final_factors_no_collinearity)
...
x_train_all = x.iloc[:train_size]
x_test = x.iloc[train_size:]

y_train_all = y.iloc[:train_size]
...
x_train, x_cv, y_train, y_cv = train_test_split(x_train_all,y_train_all,test_size=0.2,random_state=Para.seed)
params = {
...
print(f'score is {grid_search.score(x_cv,y_cv)}')
...
model = RandomForestClassifier(n_estimators = 200, max_depth = 8, random_state = Para.seed)
